...
class ErosTerminal:
    ...
    def receive_main(self, data):
        ...
        if is_error:
            ...
            if len(self.main_receive_buffer) > 0:
                self.terminal_write(
                    f"\033[91mError: {self.main_receive_buffer.decode()}\033[0m\n"
                )
            else:
                self.terminal_write("\033[91mError\033[0m\n")
        else:
            if len(self.main_receive_buffer) > 0:
                self.terminal_write(f"{self.main_receive_buffer.decode()}\n")
            else:
                self.terminal_write("\033[92mOK\033[0m\n")
        self.main_receive_buffer = b""
    ...
    def terminal_write(self, text):
        ...
        with self.terminal.location(x=0, y=0):
            print(
                self.terminal.black_on_darkkhaki(self.terminal.center("EROS Terminal"))
            )
